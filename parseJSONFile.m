...
while ~feof(fid),
    ...
    if isempty(l0),
        ...
        [data json] = parse_json(l0);
    end;
    O.Raw{length(O.Raw) + 1} = data{1};
end;
fclose(fid);
...
tmp_varnames = fieldnames(O.Raw{1});
O.D = [];
for iTrial = 1:length(O.Raw),
    newrow = [];
    for ivn = 1:length(tmp_varnames),
        val0 = getfield(O.Raw{iTrial}, tmp_varnames{ivn});
        if iscell(val0),
            for iCell = 1:length(val0),
                if isnumeric(val0{iCell}),
                    newrow = [newrow val0{iCell}];
                end;
            end;
        elseif isnumeric(val0),
            newrow = [newrow val0];
        end;
    end;
    O.D = [O.D; newrow];
...
O.varnames = {};
for ivn = 1:length(fieldnames(O.Raw{1})),
    val0 = getfield(O.Raw{iTrial}, tmp_varnames{ivn});
    if iscell(val0),
        for iCell = 1:length(val0),
            if isnumeric(val0{iCell}),
                O.varnames{end + 1} = [tmp_varnames{ivn} num2str(iCell)];
            end;
        end;
    elseif isnumeric(val0),
        O.varnames{end + 1} = tmp_varnames{ivn};
    end;
end;

O.vn2c = struct();
for ivn = 1:length(O.varnames),
    O.vn2c = setfield(O.vn2c, O.varnames{ivn}, ivn);
end;
