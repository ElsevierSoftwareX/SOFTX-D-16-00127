function [meanvec, idmat, Nvec] = rec_combo_acc(varargin)
...
if (depth == size(indepvec, 2)),
    ...
    for iu = 1:length(u),
        ...
        if length(unique(z)) > 3,
            foutlier = find(abs(z) > 2);
%             selected(foutlier) = [];
        end;
        if ~isempty(selected),
%             selected = [selected(:); 0; 1]; % add "buffer" correct and incorrect trial for normalization.
            newmeanvec = mean(selected);
%             newmeanvec = atanh(sqrt(newmeanvec));
